function [outs]=make_tree_part3(attributes,datas,class,level,column)
...
%     end
    ...
    for possibles=1:1:16
        ...
        if(c~=1)
            a=next;
         %   if(level < 3)
                next=[a,make_tree_part3(new_attributes,childs_data,class,level+1,16*(column-1)+possibles)];
          %  end
        end
    end
